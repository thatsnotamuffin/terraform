locals {
  ...
  domain_environment = "pre-prod"

  # General
  ami        = data.aws_ami.pre-prod_ami.id
  ssh_key    = "pre-prod-admin"
  ...
  db_vpc                         = "vpc-111aaa"
  alb_logs_bucket                = "thatsnotamuffin-pre-prod"
...
#- Data Sources
# AMI
data "aws_ami" "pre-prod_ami" {
  owners = ["self"]

  filter {
    name   = "tag:Name"
    values = ["thatsnotamuffin-pre-prod-ami"]
  }
}
